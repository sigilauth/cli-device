...
// Domain separation tags per api/domain-separation.md
var (
	// DomainAuth is used for authentication challenge signatures
	// Message format: challenge_bytes || SHA256(canonical_json(action_context))
	DomainAuth = []byte("SIGIL-AUTH-V1\x00")

	// DomainMPA is used for multi-party authorization signatures
	// Message format: action_context_bytes (already canonical JSON)
	DomainMPA = []byte("SIGIL-MPA-V1\x00")

	// DomainDecrypt is used for secure decrypt envelope signatures
	// Message format: decrypt_id || ciphertext
...
// ComputeActionContextHash computes SHA256(canonical_json(action_context))
// per RFC 8785. If action_context is nil or empty, returns SHA256('{}').
...
//
// The message is prefixed with the domain tag before hashing and signing.
// Returns a fixed 64-byte signature (r || s) with low-S normalization per BIP-62.
func SignWithDomain(privateKey *ecdsa.PrivateKey, domain []byte, message []byte) ([]byte, error) {
	// Domain-separated input: domain_tag || message
