...
func TestEntropyToMnemonic_InvalidLength(t *testing.T) {
	// Wrong length - should be 32 bytes
	entropy := make([]byte, 16)
	_, err := crypto.EntropyToMnemonic(entropy)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "entropy must be 32 bytes")

	// Zero length
	_, err = crypto.EntropyToMnemonic([]byte{})
	assert.Error(t, err)
}

func TestMnemonicToEntropy_InvalidCases(t *testing.T) {
	// Invalid word
	_, err := crypto.MnemonicToEntropy("invalid word list not real mnemonic test foo bar baz qux quux corge grault garply waldo fred plugh xyzzy thud")
	assert.Error(t, err)

	// Wrong length (too short)
	_, err = crypto.MnemonicToEntropy("abandon abandon abandon")
	assert.Error(t, err)

	// Empty
	_, err = crypto.MnemonicToEntropy("")
	assert.Error(t, err)
}
